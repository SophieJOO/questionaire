...
async function sendToSlack(patientData, analysis, chartOutput) {
  ...
  const blocks = [
    ...
    { type: "divider" },
    // 차트는 별도 파일로 업로드됨 (uploadChartFile 함수)
    {
      type: "context",
      elements: [
        {
  ...
  // 차트를 텍스트 파일로 업로드
  await uploadChartFile(patientData, chartOutput);
}
...
async function uploadChartFile(patientData, chartOutput) {
  ...
  if (!botToken || !channelId) {
    console.log('SLACK_BOT_TOKEN 또는 SLACK_CHANNEL_ID 미설정 - 파일 업로드 생략');
    return;
  }
  ...
  try {
    ...
    console.error('차트 파일 업로드 오류:', error.message);
  }
}
